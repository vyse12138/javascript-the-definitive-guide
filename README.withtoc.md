...
* [JS犀牛书英文第七版笔记](#js犀牛书英文第七版笔记)

	* [词法结构（Lexical Structure）](#词法结构（lexical-structure）)

		* [文本和文字（Text and Literals）](#文本和文字（text-and-literals）)

		* [注释（Comments）](#注释（comments）)

	* [类型，值和变量（Types, Values and Variables）](#类型，值和变量（types,-values-and-variables）)

		* [数字（Numbers）](#数字（numbers）)

			* [运算方法（Arithmetic）](#运算方法（arithmetic）)

			* [BigInt](#bigint)

		* [文本（Text）](#文本（text）)

			* [字符串（Stirng）](#字符串（stirng）)

			* [字符串的用法（Working with Strings)](#字符串的用法（working-with-strings))

		* [布尔值（Boolean Values）](#布尔值（boolean-values）)

		* [Null 和 Undefined（Null and Undefined）](#null-和-undefined（null-and-undefined）)

		* [Symbols](#symbols)

		* [全局对象（The Global Object）](#全局对象（the-global-object）)

		* [不可变的基本类型和可变的对象引用（Immutable Primitive Values and Mutable Object References）](#不可变的基本类型和可变的对象引用（immutable-primitive-values-and-mutable-object-references）)

		* [类型转换 (Type Conversions）](#类型转换-(type-conversions）)

			* [转换和相等 （Conversions and Equality）](#转换和相等-（conversions-and-equality）)

			* [显式转换（Explicit Conversions）](#显式转换（explicit-conversions）)

			* [引用值到基本值的转换 （Object to Primitive Conversions）](#引用值到基本值的转换-（object-to-primitive-conversions）)

				* [对象到字符串转换](#对象到字符串转换)

				* [对象到数字转换](#对象到数字转换)

				* [toString() 和 valueOf() 方法](#tostring()-和-valueof()-方法)

		* [变量声明和赋值（Variable Declaration and Assignment）](#变量声明和赋值（variable-declaration-and-assignment）)

			* [使用 let 和 const 声明](#使用-let-和-const-声明)

			* [变量和常量的作用域（VARIABLE AND CONSTANT SCOPE）](#变量和常量的作用域（variable-and-constant-scope）)

			* [重复声明](#重复声明)

			* [声明和类型](#声明和类型)

		* [使用 var 声明变量 （Variable Declarations with var）](#使用-var-声明变量-（variable-declarations-with-var）)

		* [解构赋值（Destructuring Assignment）](#解构赋值（destructuring-assignment）)

		* [小 结](#小-结)

	* [表达式和操作符（Expressions andOperators）](#表达式和操作符（expressions-andoperators）)

		* [主要表达式（Primary Expression）](#主要表达式（primary-expression）)

		* [对象和数组的初始化（Object and Array Initializers）](#对象和数组的初始化（object-and-array-initializers）)

		* [函数定义表达式（Function Definition Expressions）](#函数定义表达式（function-definition-expressions）)

		* [属性访问表达式（Property Access Expressions）](#属性访问表达式（property-access-expressions）)

			* [条件属性访问表达式（Conditional Property Access）](#条件属性访问表达式（conditional-property-access）)

		* [调用表达式（Invocation Expressions）](#调用表达式（invocation-expressions）)

			* [条件调用表达式（Conditional Invocation）](#条件调用表达式（conditional-invocation）)

		* [对象构建表达式（Object Creation Expressions）](#对象构建表达式（object-creation-expressions）)

		* [操作符概览（Operator Overview）](#操作符概览（operator-overview）)

		* [运算表达式 （Arithmetic Expressions）](#运算表达式-（arithmetic-expressions）)

			* [加法表达式（+）](#加法表达式（+）)

			* [一元表达式（Unary Arithmetic Operators）](#一元表达式（unary-arithmetic-operators）)

			* [位运算（Bitwise Operators）](#位运算（bitwise-operators）)

		* [关系表达式（Relational Expressions）](#关系表达式（relational-expressions）)
		...
		* [逻辑表达式（Logical Expressions）](#逻辑表达式（logical-expressions）)

		* [赋值表达式（Assignment with Operation）](#赋值表达式（assignment-with-operation）)
	...
	* [语句 (Statements)](#语句-(statements))
		...
		* [表达式语句（Expression Statements）](#表达式语句（expression-statements）)

		* [复合语句和空语句（Compound and Empty Statements）](#复合语句和空语句（compound-and-empty-statements）)

		* [条件句法（Conditionals）](#条件句法（conditionals）)
		...
		* [声明（Declarations）](#声明（declarations）)
			...
			* [const，let 和 var](#const，let-和-var)
	...
	* [对象（Object）](#对象（object）)
		...
		* [创建对象](#创建对象)
			...
			* [原型（prototypes）](#原型（prototypes）)

		* [Object.create()](#object.create())

		* [查询和修改属性（Querying and Setting Properties）](#查询和修改属性（querying-and-setting-properties）)

			* [将对象联想为数组（Objects As Associative Arrays）](#将对象联想为数组（objects-as-associative-arrays）)

			* [继承（Inheritance）](#继承（inheritance）)

			* [属性访问错误（Property Access Errors）](#属性访问错误（property-access-errors）)

		* [删除属性（Deleting Properties）](#删除属性（deleting-properties）)

		* [检验属性（Testing Properties）](#检验属性（testing-properties）)

		* [遍历属性（Enumerating Properties）](#遍历属性（enumerating-properties）)

			* [遍历顺序](#遍历顺序)

		* [为对象增加属性（Extending Objects）](#为对象增加属性（extending-objects）)

		* [序列化对象（Serializing Objects）](#序列化对象（serializing-objects）)

		* [对象上的方法（Object Methods）](#对象上的方法（object-methods）)

			* [toString() 方法](#tostring()-方法)

			* [toLocaleString() 方法](#tolocalestring()-方法)

			* [valueOf() 方法](#valueof()-方法)

			* [toJSON() 方法](#tojson()-方法)

		* [更多的对象字面量句法（Extended Object Literal Syntax）](#更多的对象字面量句法（extended-object-literal-syntax）)

			* [属性简写（Shorthand Properties）](#属性简写（shorthand-properties）)

			* [计算生成的属性名（Computed Property Names）](#计算生成的属性名（computed-property-names）)

			* [Symbol 作为属性名（Symbols as Property Names）](#symbol-作为属性名（symbols-as-property-names）)

			* [展开操作符（Spread Operator）](#展开操作符（spread-operator）)

			* [方法简写（Shorthand Methods）](#方法简写（shorthand-methods）)

			* [属性的 Getters 和 Setters](#属性的-getters-和-setters)

		* [小结](#小结)

	* [数组（Arrays）](#数组（arrays）)

		* [创建数组（Creating Arrays）](#创建数组（creating-arrays）)

			* [数组字面量（Array Literals）](#数组字面量（array-literals）)

			* [展开操作符（The Spread Operator）](#展开操作符（the-spread-operator）)

			* [数组构造函数（The Array() Constructor）](#数组构造函数（the-array()-constructor）)

			* [Array.of()](#array.of())

			* [Array.from()](#array.from())

		* [读写数组元素（Reading and Writing Array Elements）](#读写数组元素（reading-and-writing-array-elements）)

		* [稀疏数组（Sparse Arrays）](#稀疏数组（sparse-arrays）)

		* [数组长度（Array Length）](#数组长度（array-length）)

		* [增删数组元素（Adding and Deleting Array Elements）](#增删数组元素（adding-and-deleting-array-elements）)

		* [遍历数组（Iterating Arrays）](#遍历数组（iterating-arrays）)

		* [多维数组（Multidimensional Arrays）](#多维数组（multidimensional-arrays）)

		* [数组方法（Array Methods）](#数组方法（array-methods）)
			...
			* [数组模拟栈和队列](#数组模拟栈和队列)
				...
				* [push() 和 pop()](#push()-和-pop())

				* [unshift() 和 shift()](#unshift()-和-shift())
		...
		* [类数组对象（Array-Like Objects）](#类数组对象（array-like-objects）)
	...
	* [函数（Functions）](#函数（functions）)
		...
		* [定义函数（Defining Functions）](#定义函数（defining-functions）)

			* [函数声明（Function Declarations）](#函数声明（function-declarations）)

			* [函数表达式（Function Expressions）](#函数表达式（function-expressions）)

			* [箭头函数（Arrow Function）](#箭头函数（arrow-function）)

			* [嵌套函数（Nested Functions）](#嵌套函数（nested-functions）)

		* [调用函数（Invoking Functions）](#调用函数（invoking-functions）)

			* [函数调用（Function Invocation）](#函数调用（function-invocation）)

			* [方法调用（Method Invocation）](#方法调用（method-invocation）)

				* [this 指向](#this-指向)

			* [构造函数调用（Constructor Invocation）](#构造函数调用（constructor-invocation）)

			* [非直接调用（Indirect Invocation）](#非直接调用（indirect-invocation）)

			* [隐式函数调用（ Implicit Function Invocation）](#隐式函数调用（-implicit-function-invocation）)

		* [函数的 arguments 和参数（Function Arguments and Parameters）](#函数的-arguments-和参数（function-arguments-and-parameters）)

			* [可选参数和默认值（Optional Parameters and Defaults）](#可选参数和默认值（optional-parameters-and-defaults）)

			* [剩余参数（Rest Parameters）](#剩余参数（rest-parameters）)

			* [arguments 对象（The Arguments Object）](#arguments-对象（the-arguments-object）)

			* [在调用函数时使用展开操作符（The Spread Operator for Function Calls）](#在调用函数时使用展开操作符（the-spread-operator-for-function-calls）)

			* [解构 arguments 进入参数（Destructuring Function Arguments intoParameters）](#解构-arguments-进入参数（destructuring-function-arguments-intoparameters）)

			* [argument 类型（Argument Types）](#argument-类型（argument-types）)

		* [函数作为值（Functions as Values）](#函数作为值（functions-as-values）)

				* [定义函数属性](#定义函数属性)

		* [函数作为命名空间（Functions as Namespaces）](#函数作为命名空间（functions-as-namespaces）)

		* [闭包（Closures）](#闭包（closures）)

		* [函数的属性和方法（Function Properties, Methods, and Constructor）](#函数的属性和方法（function-properties,-methods,-and-constructor）)
			...
			* [call() 和 apply() 方法](#call()-和-apply()-方法)
	...
	* [类（Classes）](#类（classes）)
		...
		* [类和原型（Classes and Prototypes）](#类和原型（classes-and-prototypes）)

		* [类和构造函数（Classes and Constructors）](#类和构造函数（classes-and-constructors）)

		* [用 class 关键字构造类（Classes with the class Keyword）](#用-class-关键字构造类（classes-with-the-class-keyword）)

			* [静态方法](#静态方法)

			* [Getters，Setters](#getters，setters)

		* [为现有的类增添方法（Adding Methods to Existing Classes）](#为现有的类增添方法（adding-methods-to-existing-classes）)

		* [子类（Subclasses）](#子类（subclasses）)

			* [使用 extends 和 super 关键字](#使用-extends-和-super-关键字)

			* [用组合取代继承（ Delegation Instead of Inheritance）](#用组合取代继承（-delegation-instead-of-inheritance）)

		* [小结](#小结)

	* [模块（Modules）](#模块（modules）)

		* [Node 中的模块（Modules in Node）](#node-中的模块（modules-in-node）)

			* [Node 中的 Exports](#node-中的-exports)

			* [Node 中的 Imports](#node-中的-imports)

		* [ES6 中的模块（Modules in ES6）](#es6-中的模块（modules-in-es6）)

			* [ES6 中的 Exports](#es6-中的-exports)

			* [ES6 中的 Imports](#es6-中的-imports)

			* [Re-Exports](#re-exports)

			* [网络上的 JavaScript 模块](#网络上的-javascript-模块)

		* [使用 import() 进行动态加载](#使用-import()-进行动态加载)

			* [import.meta.url](#import.meta.url)

		* [小结：](#小结：)

	* [JavaScript 标准库（The JavaScript Standard Library）](#javascript-标准库（the-javascript-standard-library）)

		* [Set 和 Map（Sets and Maps）](#set-和-map（sets-and-maps）)

			* [Set 类](#set-类)

			* [Map 类](#map-类)

			* [WeakMap 类和 WeakSet 类](#weakmap-类和-weakset-类)

		* [类型化数组和二进制文件（Typed Arrays and Binary Data）](#类型化数组和二进制文件（typed-arrays-and-binary-data）)

			* [Typed Array 类型](#typed-array-类型)

			* [创建 Typed Array](#创建-typed-array)

			* [使用 Typed Array](#使用-typed-array)

			* [Typed Array 的属性和方法](#typed-array-的属性和方法)

			* [字节序和 DataView 类](#字节序和-dataview-类)

		* [正则表达式（Pattern Matching with Regular Expressions）](#正则表达式（pattern-matching-with-regular-expressions）)

			* [定义正则表达式（Defining Regular Expressions）](#定义正则表达式（defining-regular-expressions）)

				* [字面量字符（LITERAL CHARACTERS）](#字面量字符（literal-characters）)

				* [字符类（CHARACTER CLASSES）](#字符类（character-classes）)

				* [重复（REPETITION）](#重复（repetition）)

				* [非贪婪重复（NON-GREEDY REPETITION）](#非贪婪重复（non-greedy-repetition）)

				* [选项，组合和引用(ALTERNATION, GROUPING, AND REFERENCES)](#选项，组合和引用(alternation,-grouping,-and-references))

				* [指定匹配位置（SPECIFYING MATCH POSITION）](#指定匹配位置（specifying-match-position）)

				* [标志（FLAGS）](#标志（flags）)

			* [使用正则表达式的字符串方法（String Methods for Pattern Matching）](#使用正则表达式的字符串方法（string-methods-for-pattern-matching）)
				...
				* [matchAll() 方法](#matchall()-方法)

				* [split() 方法](#split()-方法)

			* [RegExp 类（The RegExp Class）](#regexp-类（the-regexp-class）)

				* [RegExp 的构造函数](#regexp-的构造函数)

				* [RegExp 的属性](#regexp-的属性)

				* [RegExp 的方法](#regexp-的方法)

		* [日期和时间（Dates and Times）](#日期和时间（dates-and-times）)

			* [时间戳（Timestamps）](#时间戳（timestamps）)

			* [日期运算（Date Arithmetic）](#日期运算（date-arithmetic）)

		* [格式化日期（Formatting and Parsing Date Strings）](#格式化日期（formatting-and-parsing-date-strings）)

		* [Error 类（Error Classes）](#error-类（error-classes）)

		* [JSON 的序列化和解析（JSON Serialization and Parsing）](#json-的序列化和解析（json-serialization-and-parsing）)

			* [自定义 JSON](#自定义-json)

		* [The Internationalization API](#the-internationalization-api)

			* [格式化数字（Formatting Numbers）](#格式化数字（formatting-numbers）)

			* [格式化日期和时间（Formatting Dates and Times）](#格式化日期和时间（formatting-dates-and-times）)
	...
	* [迭代器和生成器（Iterators and generators）](#迭代器和生成器（iterators-and-generators）)

		* [迭代器是怎么工作的（How Iterators Work）](#迭代器是怎么工作的（how-iterators-work）)

		* [实现一个可迭代对象（Implementing Iterable Objects）](#实现一个可迭代对象（implementing-iterable-objects）)

			* [终止迭代器（“Closing” an Iterator: The Return Method）](#终止迭代器（“closing”-an-iterator:-the-return-method）)

		* [生成器（Generators）](#生成器（generators）)

		* [进阶生成器特性（Advanced Generator Features）](#进阶生成器特性（advanced-generator-features）)

			* [生成器函数的返回值](#生成器函数的返回值)

			* [yield 表达式的值](#yield-表达式的值)

			* [return() 和 throw() 方法](#return()-和-throw()-方法)

		* [小结](#小结)

	* [异步 JavaScript（Asynchronous JavaScript）](#异步-javascript（asynchronous-javascript）)

		* [使用回调函数进行异步编程（Asynchronous Programming with Callbacks）](#使用回调函数进行异步编程（asynchronous-programming-with-callbacks）)

			* [计时器（Timers）](#计时器（timers）)

			* [事件（Events）](#事件（events）)

			* [网络事件（Network Events）](#网络事件（network-events）)

		* [Promises](#promises)

			* [使用 Promises](#使用-promises)

			* [使用 Promises 处理异常](#使用-promises-处理异常)

			* [链式调用 Promise](#链式调用-promise)

			* [Resolving Promise](#resolving-promise)

			* [Promises 和 Error](#promises-和-error)

			* [同时执行多个 Promises](#同时执行多个-promises)

			* [创建 Promises](#创建-promises)

				* [基于其他 Promises 的 Promises](#基于其他-promises-的-promises)

				* [基于同步值的 Promises](#基于同步值的-promises)

				* [从头开始创建 Promises](#从头开始创建-promises)

		* [aysnc 和 await](#aysnc-和-await)

			* [await 表达式](#await-表达式)

			* [async 函数](#async-函数)

		* [异步迭代（ Asynchronous Iteration）](#异步迭代（-asynchronous-iteration）)

			* [for await 循环](#for-await-循环)
	...
	* [元编程（Metaprogramming）](#元编程（metaprogramming）)
		...
		* [属性的特性（Property Attributes）](#属性的特性（property-attributes）)

		* [对象的可扩展性（Object Extensibility）](#对象的可扩展性（object-extensibility）)

		* [原型特性（The prototype Attribute）](#原型特性（the-prototype-attribute）)

		* [Well-Known Symbols](#well-known-symbols)

			* [Symbol.iterator 和 Symbol.asyncIterator](#symbol.iterator-和-symbol.asynciterator)
	...
	* [浏览器中的 JavaScript（JavaScript in Web Browsers）](#浏览器中的-javascript（javascript-in-web-browsers）)

		* [网页编程基础（Web Programming Basics）](#网页编程基础（web-programming-basics）)

			* [HTML \<script\> 标签中的 JavaScript](#html-\<script\>-标签中的-javascript)

			* [DOM（Document Object Model）](#dom（document-object-model）)

			* [浏览器中的全局对象](#浏览器中的全局对象)

			* [JavaScript 程序的执行](#javascript-程序的执行)

				* [客户端 JavaScript 线程 model](#客户端-javascript-线程-model)

				* [客户端 JavaScript 时间轴](#客户端-javascript-时间轴)
		...
		* [事件（Events）](#事件（events）)
			...
			* [注册事件处理器](#注册事件处理器)
				...
				* [设置事件处理属性（JavaScript 中）](#设置事件处理属性（javascript-中）)

				* [设置事件处理属性（HTML 中）](#设置事件处理属性（html-中）)

				* [addEventListener()](#addeventlistener())

				* [事件处理函数调用](#事件处理函数调用)

				* [事件处理函数上下文（context）](#事件处理函数上下文（context）)

				* [事件处理函数的返回值](#事件处理函数的返回值)

			* [事件传播（Event Propagation）](#事件传播（event-propagation）)

			* [事件取消](#事件取消)

			* [生成自定义事件](#生成自定义事件)

		* [Scripting Documents](#scripting-documents)

			* [选择 Document 元素](#选择-document-元素)

				* [通过 CSS 选择器选择元素](#通过-css-选择器选择元素)

				* [其他的元素选择方法](#其他的元素选择方法)

				* [预设元素选择](#预设元素选择)

			* [Document 的结构和遍历](#document-的结构和遍历)

			* [元素属性](#元素属性)

				* [以 ELement 对象属性表示 HTML 属性](#以-element-对象属性表示-html-属性)
			...
			* [元素内容](#元素内容)
				...
				* [以 HTML 作为元素内容](#以-html-作为元素内容)

				* [以纯文本作为元素内容](#以纯文本作为元素内容)

			* [创建，插入，删除节点](#创建，插入，删除节点)

		* [Scripting CSS](#scripting-css)
		...
		* [Document 的滚动和几何学（Document Geometry and Scrolling）](#document-的滚动和几何学（document-geometry-and-scrolling）)

			* [Document 坐标系和视口坐标系](#document-坐标系和视口坐标系)

			* [查询元素的几何属性](#查询元素的几何属性)

			* [查询一个点对应的元素](#查询一个点对应的元素)

			* [滚动](#滚动)

			* [视口大小，内容大小以及滚动位置](#视口大小，内容大小以及滚动位置)

		* [网页组件（Web Components）](#网页组件（web-components）)

			* [使用网页组件](#使用网页组件)

			* [HTML Templates](#html-templates)

			* [自定义 Elements](#自定义-elements)

			* [Shadow DOM](#shadow-dom)

				* [Shadow DOM 封装](#shadow-dom-封装)

				* [Shadow DOM 的 Slots 和 Light DOM 的 Children](#shadow-dom-的-slots-和-light-dom-的-children)

				* [SHADOW DOM API](#shadow-dom-api)

		* [Location，Navigation 和 History](#location，navigation-和-history)

			* [加载新的 Document](#加载新的-document)

			* [浏览器历史记录](#浏览器历史记录)

				* [通过 hashchange 进行管理](#通过-hashchange-进行管理)

				* [通过 pushState() 进行管理](#通过-pushstate()-进行管理)

		* [网络（Networking）](#网络（networking）)

			* [fetch()](#fetch())

				* [HTTP 状态码，响应头以及网络异常](#http-状态码，响应头以及网络异常)

				* [设置请求头](#设置请求头)

				* [解析响应体](#解析响应体)

				* [Streaming 响应体](#streaming-响应体)
			...
			* [WebSockets](#websockets)
				...
				* [与 WebSocket 建立连接](#与-websocket-建立连接)

				* [通过 WebSocket 收发消息](#通过-websocket-收发消息)
		...
		* [本地存储](#本地存储)
			...
			* [Cookies](#cookies)
				...
				* [读取 cookies](#读取-cookies)

				* [cookies 的特性](#cookies-的特性)

				* [cookies 的限制](#cookies-的限制)

				* [保存 cookies](#保存-cookies)
		...
		* [Worker 线程](#worker-线程)
			...
			* [Workers 中的全局对象](#workers-中的全局对象)

			* [在 Worker 中 import 代码](#在-worker-中-import-代码)

			* [Worker 的信息传输](#worker-的信息传输)
...
客户端的 JavaScript 程序使用了异步的，事件驱使的编程模型。在这种模型之下，浏览器会在某些事情发生时，生成一个特定的事件。因为事件可以在任何 HTML document 中的元素上生成，所以浏览器中的事件会比 Node 中的复杂许多。下面是对事件的一些介绍：
...
这是一个对象，它和一个事件以及其内容关联的，事件对象以 argument 的形式传入事件处理函数。事件对象拥有 tyep 和 target 属性用于指定类型和目标。事件类型又会为其事件对象定义一系列的属性。比如鼠标移动的事件对象会持有鼠标的坐标。而某些事件类型则不会未事件对象定义属性，使得事件对象只拥有 type 和 target 这两个属性。
...
这一类输入事件则不会和设备绑定，比如 click 事件，虽然通常是使用鼠标点击，不过也可以通过键盘或者点击触摸屏来完成。比如：pointerdown，pointermove，pointerup 就不会和设备绑定，对于触摸屏，触摸板等也能生效。
...
每一个事件处理函数会接收一个事件对象作为其单一的 argument，这个事件对象包括以下属性：
...
取消与事件关联的默认 action 是唯一一种事件取消技术。我们也可以通过调用事件对象上的 stopPropagation() 方法来取消事件的传播。对于同一目标对象上的事件处理函数，它们依然会被调用，
但是对于其他对象上的处理函数则不会再被调用。

stopPropagation() 方法会在捕获阶段以及冒泡阶段都可以生效，调用时要记得以 e.stopPropagation() 的形式，因为该方法在事件对象上。

还有一个类似的方法 stopImmediatePropagation()，不过它会同时阻止目标对象上剩余的事件处理函数的执行。
...
- 介绍了如何将 JavaScript 引入 HTML
- 客户端 JavaScript 中异步的，事件驱使的执行特性
- DOM 以及与其关联的 APIs
- JavaScript 如何操纵 CSS
- 如何操纵当前展示页面的部分
- 如何创建可复用网页组件
- 如何使用 JavaScript 来操纵浏览器历史
- 通过 HTTP 和 WebSocket 协议来进行与服务器的通信
- JavaScript 如何进行本地储存的
- 如何通过 Worker 执行多线程操作